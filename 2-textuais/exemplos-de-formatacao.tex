...
Esta seção reúne exemplos deliberadamente variados para que o documento de referência também funcione como corpus visual de regressão. Os elementos abaixo são exemplos e devem ser substituídos pelo conteúdo acadêmico do trabalho.
...
\begin{ufcobjeto}[here]
  \centering
  \IfFileExists{figuras/ufc-campus-pici.jpg}
    {\includegraphics[width=0.72\linewidth]{figuras/ufc-campus-pici.jpg}}
...
\ufcnota{Fotografia externa usada sem modificação e distribuída sob licença própria; ver \texttt{figuras/LICENCAS.md}.}
\label{fig:reitoria-ufc}
\begin{ufcobjeto}[here]
  \centering
  \IfFileExists{figuras/ufc-reitoria.jpg}
    {\includegraphics[width=0.62\linewidth]{figuras/ufc-reitoria.jpg}}
...
As Figuras~\ref{fig:campus-pici-computacao} e~\ref{fig:reitoria-ufc} exercitam fotografias reais com proporções e dimensões distintas. As imagens são baixadas por \texttt{make reference-assets}, com verificação de integridade e atribuição registrada em \texttt{figuras/LICENCAS.md}.
...
O Gráfico~\ref{graf:barras} valida a inclusão de JPEG, a numeração de gráficos e sua presença na Lista de Ilustrações. O arquivo de exemplo contém eixos, categorias, escala e valores explícitos para evitar que uma regressão visual seja confundida com um gráfico vazio.
...
\subsection{Referências bibliográficas e recursos eletrônicos}

O arquivo \texttt{3-pos-textuais/referencias.bib} reúne exemplos de tipos bibliográficos distintos para demonstrar a composição final das referências. Além das normas e documentos institucionais citados no texto, o corpus inclui livro, artigo de periódico com DOI, trabalho publicado em evento, dissertação, relatório técnico e recursos eletrônicos com URL e data de acesso.

Os exemplos também exercitam campos úteis do \texttt{biblatex}, como \texttt{isbn}, \texttt{doi}, \texttt{url}, \texttt{urldate}, \texttt{institution}, \texttt{edition}, \texttt{volume}, \texttt{number} e \texttt{pages}. Em trabalhos reais, devem constar nas Referências somente as fontes efetivamente citadas ou utilizadas de acordo com a política bibliográfica adotada.
