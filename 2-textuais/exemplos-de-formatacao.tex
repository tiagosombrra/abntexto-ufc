...
\subsection{Figuras em diferentes larguras}
...
As Figuras~\ref{fig:media} e~\ref{fig:larga} exercitam larguras progressivamente maiores sem alterar a API pública do objeto.
...
\begin{ufcobjeto}[here]
  \centering
  \fbox{%
    \parbox[c][3.2cm][c]{9cm}{%
      \centering
      \rule{1cm}{1.0cm}\hspace{0.8cm}
      \rule{1cm}{1.8cm}\hspace{0.8cm}
      \rule{1cm}{2.5cm}\\[0.4em]
      A\hspace{1.55cm}B\hspace{1.55cm}C
    }%
  }
\end{ufcobjeto}

O Gráfico~\ref{graf:barras} valida a numeração e a inclusão de gráficos na Lista de Ilustrações.
