...
A representação visual da ficha catalográfica é controlada pela opção \texttt{ficha-catalografica}. No documento de referência ela permanece desativada para demonstrar o comportamento padrão adotado na V2.

\guiainstitucional{A política de depósito da UFC vigente em 2026 tornou facultativa a representação visual da ficha catalográfica no arquivo depositado para os tipos documentais abrangidos pela instrução normativa correspondente. O autor deve conferir o procedimento institucional vigente no momento da entrega.}
...
\guiaexemplo{Se não houver correções a registrar, remova a chamada \texttt{\string\imprimirerrata} do documento. O arquivo de exemplo existe para demonstrar a composição quando o elemento é necessário.}
...
\guiainstitucional{No fluxo de depósito considerado pelo projeto, a folha de aprovação é apresentada sem reproduzir assinaturas digitalizadas. A conferência final deve considerar a orientação vigente do Sistema de Bibliotecas e do programa responsável.}
...
\guiaexemplo{O arquivo \texttt{1-pre-textuais/dedicatoria.tex} contém apenas uma frase curta para mostrar a posição do bloco. Em um trabalho real, substitua-a pela dedicatória desejada ou remova o elemento.}
...
\guiainstitucional{Quando o trabalho tiver financiamento ao qual se aplique obrigação específica de agradecimento, como a política da CAPES, o autor deve inserir o texto institucional exigido pela fonte correspondente. O template não presume automaticamente a existência de financiamento.}
