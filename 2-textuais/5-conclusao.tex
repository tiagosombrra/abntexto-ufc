...
A opção \texttt{tipo} seleciona o perfil acadêmico. A matriz de certificação da V2 exercita TCC de graduação, TCC de especialização, dissertação, tese, projeto de pesquisa e projeto de pesquisa anonimizado.
...
\guianormativa{ABNT NBR 15287:2025}{Os perfis \texttt{projeto} e \texttt{projetoanonimizado} usam a estrutura de projeto de pesquisa adotada pela V2. O perfil anonimizado adiciona uma política técnica de ocultação de dados pessoais para processos que exijam avaliação sem identificação; essa anonimização não é apresentada como regra geral da NBR 15287 \parencite{abnt15287}.}

\subsection{Fontes e motores de compilação}

A opção \texttt{fonte=times|arial} seleciona a família textual desejada. A opção \texttt{fonte-estrita} determina se a identidade literal é obrigatória ou se o ambiente pode usar fallback portátil.
...
O cabeçalho de \texttt{documento.tex} declara \texttt{pdfstandard=A-2b}. A suíte verifica o PDF gerado com veraPDF e também confere incorporação de fontes.

\guiainstitucional{O fluxo de depósito acompanhado pelo projeto exige PDF/A nos casos institucionais aplicáveis. O autor deve verificar a orientação vigente para seu tipo de trabalho no momento do depósito.}

\guiapolitica{A escolha do perfil específico PDF/A-2b para o documento de referência e para a certificação da V2 é uma política técnica do projeto; ela não é apresentada como uma afirmação de que toda norma ABNT exige especificamente PDF/A-2b.}

\subsection{Referências}

As referências aparecem imediatamente após o texto quando produzidas por \texttt{\string\imprimirreferencias}. O banco bibliográfico é declarado com \texttt{\string\ufcbibliografia} e processado por Biber no fluxo recomendado.
...
\guianormativa{estrutura pós-textual adotada pela V2}{Cada apêndice inicia em página própria. No escopo auditado, seu cabeçalho é centralizado, em letras maiúsculas, negrito e tamanho 12.}
...
\guianormativa{estrutura pós-textual adotada pela V2}{Cada anexo inicia em página própria e usa a mesma política de apresentação do cabeçalho exercitada para apêndices.}
...
\guianormativa{ABNT NBR 6034:2004}{O índice é elemento opcional e possui norma específica de apresentação. A V2 demonstra o recurso com \texttt{imakeidx} e verifica o título e sua composição no PDF final.}
