...
**V3-R1 ACTIVE — R1-BLOCK-4 active; B4-A done; B4-B done; B4-C active (C1 done, C2 active).**
...
**B4-A DONE → B4-B DONE → B4-C ACTIVE [C1 DONE → C2 ACTIVE] → B4-D PENDING**

- Canonical repository: `tiagosombrra/abntexto-ufc`.
- Active trunk: `main`.
- B4 operational issue: #187.
- Latest certified implementation checkpoint: **`97c808081f5a964498b7a3e71d902aeb8a9bbcf8`**.
...
**ACTIVE.** Normalize project-owned human-facing technical text while preserving academic/normative labels and all stable machine identifiers. Candidate groups include:

- generic technical categories such as metadata, integrity, typography and accessibility;
- technical levels such as automatic, geometric, typographic, heuristic, deep and conditional;
- evidence/correction diagnostics generated by Web/CLI;
- report/table/export headings and generic Web controls/disclosures.

Academic element names such as `Capa`, `Folha de aprovação`, `Resumo`, `Sumário` and `Referências` remain Portuguese domain terms unless a later explicit localization policy says otherwise.
...
Start **B4-C2** from canonical `main` with implementation checkpoint `97c808081f5a964498b7a3e71d902aeb8a9bbcf8`. Audit the remaining project-owned human-facing technical strings and translate only categories/levels/diagnostics/report/export/UI text whose ownership and consumers are clear. Preserve academic/normative terms, stable machine contracts and the Portuguese LaTeX API; validate Web/CLI producer-consumer alignment proportionally and remove temporary executors before checkpoint.
