...
**Core Corrections is ACTIVE at the corrected phase-end regression candidate gate.**
...
Rejected candidate `3b2476371e1df5180d8ee25ea53aed6a13fa2da2` failed Static `33981960024` because the machine state no longer used the contractually required `phase_end_regression.candidate = one-immutable-sha`. The governance gate worked as intended; no test is weakened.
...
| **Core Corrections** | ACTIVE — CORRECTED PHASE-END CANDIDATE | Correct shared runtime, template, normative mapping, documentation and tests identified by the audit. | Immutable candidate passes Static/full Linux with phase-specific evidence green. |
...
The corrected candidate restores the machine invariant `candidate = one-immutable-sha`. The commit containing this synchronized correction is the new immutable phase-end candidate. It must pass Static contract and full Linux integration on the same SHA. Only then may Core Corrections close.
