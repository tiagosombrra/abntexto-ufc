...
Every **material advance** that changes authority, modality, evidence ownership, validation mode, proof disposition or a factual runtime observation represented here must synchronize this contract in the same work cycle. Historical article execution progress is retained under `docs/history/v3/evidence/`; current material advances are tracked through `docs/V3.0.2-REPOSITORY-HYGIENE-STATUS.md`, applicable durable contracts, and issue #313.
