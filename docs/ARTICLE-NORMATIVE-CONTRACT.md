...
Every **material advance** that changes authority, modality, evidence ownership, validation mode, proof disposition or a factual runtime observation represented here must synchronize this contract in the same work cycle. Historical article execution progress is retained under `docs/history/v3/evidence/`; current material advances are governed by applicable durable contracts plus `docs/RELEASE-STATE.md` and current Git facts.
