...
The R4 source closes both entry defects: `validator/normative-catalog.js` is generated/tracked and byte-compared against the authoritative generator; the source gate proves relative-module closure; and Linux `web-lite` snapshots the real canonical/reference PDF and drives the productive UI in Chrome with positive and non-A4 negative PDFs. Static #491 passed. Linux #412 passed the three container checks but timed out creating the local ChromeDriver session. Corrective head `532a7996613fea0039025d073d00d53e63df707e` raised only the session-start budget, retained exact PDF input hashes and passed Static #492; Linux #413 again passed `SCOPE=web-lite PASS=3 FAIL=0 SKIP=0` but failed before PDF analysis because ChromeDriver did not become ready and its intended log file was absent. Release Check #127 passed on that same source. The next correction creates the diagnostic file before process spawn, redirects ChromeDriver stdout/stderr directly into it and detects early process exit with a retained log tail. PDF assertions, normative predicates and Deep-only `MANUAL REVIEW` boundaries remain unchanged. Evidence contract: `docs/V3.0.1-R4-EVIDENCE.md`.
