...
Updated: 2026-09-10
Status: RELEASE — R1.3 heading + bounded CI-scope correction pending fresh evidence; publication blocked
...
- Root cause: an R1.3 subsection heading contained the executable `\ufcsetup` command. `main.toc` replay executed it and produced unknown key `ufc/\check@icr`.
- The heading is now plain `Configuração central do documento`; `\ufcsetup` remains only in body text. A source comment records that executable commands must stay out of this moving heading.
- Static #481 (`34484253436`) passed on the corrected heading head `ba8aeb1...`.
- Linux #402 (`34484253425`) began a real integration run but selected a broad scope because the active machine-state JSON was treated as an unknown technical path. The successor control-plane correction intentionally supersedes that run for R1.3 closure.

## Bounded CI-scope correction

`release/v3.0.1-final-corrections.json` is required machine-readable documentation/control state. Updating that file alone must not launch heavy integration. The selector now treats it as docs-only, matching the existing treatment of `release/v3-roadmap.json` and `docs/**`.

Fail-closed behavior remains: unknown technical files still select `complete`; runtime/core/standards changes retain required scopes; final/release candidate markers force `complete`; orchestration-only changes select `smoke`; and the corrected TCC chapter selects `reference-document`. The selector self-test and `tests/checks/linux_integration_suites.py` protect this rule.

Current next action: resolve the current branch HEAD dynamically and require fresh Static PASS plus a real Linux run in which `reference` passes. Only then mark R1.3 DONE and begin R2.
