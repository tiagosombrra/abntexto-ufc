...
O modo `release` acrescenta as certificações profundas de PDF/A e é exposto por `make release-check` e `make release-preflight`. Os scripts `tests/v2-*` permanecem como implementações de baixo nível consumidas pelo runner e por certificações de plataforma específicas. Após equivalência comprovada no mesmo SHA, os wrappers redundantes `v2-*` do Makefile foram removidos; permanecem apenas os atalhos direcionados ainda documentados ou consumidos pelo Gate T. A distribuição também reutiliza os PDFs produzidos pelo preflight unificado, sem recompilar novamente o documento de referência e a matriz de perfis antes da certificação PDF/A.
