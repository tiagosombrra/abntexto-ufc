...
The object-typography migration began at `f2f5124c4adcb34069a667f1ef80c76fb17728bd`. Branch-level Static run `33963240056` passed, while full Linux run `33963240297` correctly exposed a remaining `tabularray-abnt` compatibility adapter that still forced table identification to 10 pt. Commit `7ec385ebecf21ba17e59db1e7ec16d3336f4bf4c` corrected that runtime residual.

The next branch checkpoint `faa487ed38ca130c9eb9da597d2902603f269a0a` passed Static run `33964421654`. Full Linux run `33964421597` reached the corrected object evidence and confirmed all four relevant final-PDF measurements: illustration identification 12 pt, illustration source 10 pt, table identification 12 pt, table source 10 pt. It then failed one independent legacy assertion in `tests/integration/table-ibge.sh`, which still expected the historical 10 pt table caption. Commit `a3ce2d82899162d12b06c7335b149dc2b44ecfa3` aligns that stale observer with the accepted 12 pt/10 pt contract and also converts its project-owned technical diagnostics to English.

Review item 21 therefore remains `FAIL` only until a normal branch checkpoint containing `a3ce2d8...` passes Static contract and full Linux integration. The implementation decision and final-PDF observer are already consistent; the remaining gate is regression acceptance.
...
| 21 | Figure/table/object upper identification/title must use body-size typography (12 pt); lower legend/source/note remain reduced where applicable. | FAIL — final-PDF evidence in Linux run `33964421597` already confirms the required 12 pt/10 pt split; the only failure was a stale legacy IBGE assertion, corrected at `a3ce2d8...`. Close only after the next normal Static/full Linux regression is green. | `objects.def`, `modules.def`, `tests/integration/table-ibge.sh`, normative contract, object final-PDF checks |
...
The initial migration at `f2f5124c4adcb34069a667f1ef80c76fb17728bd` retired the two semantically incorrect reduced-title rule IDs, recorded their replacement mapping in `standards/rule-migrations.json`, separated illustration/table locator ownership, and updated the shared object runtime plus final-PDF measurement expectations. Full Linux run `33963240297` then exposed a second independent styling surface: the `tabularray-abnt` compatibility adapter still forced table captions to reduced size. Commit `7ec385ebecf21ba17e59db1e7ec16d3336f4bf4c` corrected that runtime residual.

Full Linux run `33964421597` subsequently confirmed the new final-PDF contract itself: illustration title 12 pt, illustration source 10 pt, table title 12 pt, table source 10 pt. Its single remaining failure came from `tests/integration/table-ibge.sh`, which still encoded the historical 10 pt table-caption expectation. Commit `a3ce2d82899162d12b06c7335b149dc2b44ecfa3` updates that stale observer to 12 pt while preserving 10 pt source/note expectations. The test contract is therefore being aligned to the resolved authority decision, not weakened.
