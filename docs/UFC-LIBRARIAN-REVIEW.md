...
The Front Matter and Annex Closeout checkpoint `6d7a8fb8c7005030f5e1d64a42152d0364fa68c8` passed Static `33980847191` and full Linux `33980847189`, with `PASS=31 FAIL=0 SKIP=0`. The Linux run emitted explicit `LIBRARIAN-REVIEW-EVIDENCE` PASS records for items 1, 2, 7 and 34. The earlier Static failure `33980486317` remains classified as an engineering-language diagnostic-only guardrail discovery and was corrected without changing runtime or evidence predicates.

The consolidated review state is now **33 PASS, 0 PARTIAL, 0 FAIL, 1 NORMATIVE-REVIEW = 34 items**.
...
|---:|---|---|---|
| 1 | Department/unit line must be optional (`se houver`) and omitted cleanly when absent. | PASS — Linux `33980847189` proved blank department omitted and filled department rendered. | `core.def`, `academic-works.def`, cover evidence |
| 2 | Pre-textual author field/examples must make clear that the complete author name is required. | PASS — canonical generated output renders `NOME COMPLETO DO AUTOR`; Linux `33980847189`. | `template/main.tex`, reference document gate |
...
| 7 | Committee member institution must support the `Instituição (sigla)` presentation where applicable. | PASS — approval-page evidence preserves `Instituição Externa de Teste (IET)`; Linux `33980847189`. | `frontmatter.def`, approval-page evidence |
...
| 10 | `RESUMO` must begin at the first usable text line/heading position instead of being vertically displaced. | PASS — automated geometry remains green; retain presentation-phase visual confirmation. | `frontmatter.def`, front-matter geometry tests |
...
| 15 | Appendix and annex entries in the TOC must use the required uppercase/bold presentation. | PASS — retain presentation-phase visual confirmation. | appendix/annex integration and checks |
...
| 21 | Figure/table/object upper identification/title must use body-size typography (12 pt); lower legend/source/note remain reduced where applicable. | PASS | `objects.def`, object/IBGE final-PDF checks |
...
| 34 | Appendix/annex headings must use the required bold presentation, and annexed external material must explicitly identify its source. | PASS — canonical source/heading/TOC evidence plus independent bold-heading final-PDF evidence passed in Linux `33980847189`. | appendix/annex integration, canonical annex/reference gate |
