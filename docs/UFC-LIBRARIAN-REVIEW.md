...
|---:|---|---|---|
| 1 | Department/unit line must be optional (`se houver`) and omitted cleanly when absent. | PASS — automated cover evidence plus canonical page-1 visual reconfirmation. | `abntexto-ufc.cls` (core/academic-work sections), cover evidence |
...
| 10 | `RESUMO` must begin at the first usable text line/heading position instead of being vertically displaced. | PASS — automated geometry and canonical page-8 visual review agree. | `abntexto-ufc.cls` (front-matter section), front-matter geometry tests |
...
| 21 | Figure/table/object upper identification/title must use body-size typography (12 pt); lower legend/source/note remain reduced where applicable. | PASS — measured 12/10 pt and visually reconfirmed. | `abntexto-ufc.cls` (academic-objects section), object/IBGE final-PDF checks |
| 22 | Object title, source, and note blocks must use single spacing. | PASS | `abntexto-ufc.cls` (academic-objects section), object geometry checks |
| 23 | Object source indication should include a page locator when applicable. | PASS | documentary-source fixture/check |
| 24 | Alínea items begin with lowercase text when grammatically continuing the introductory sentence. | PASS | `abntexto-ufc.cls` (layout section), reference fixture |
| 25 | Alínea items use semicolons between intermediate items and appropriate final punctuation. | PASS | `abntexto-ufc.cls` (layout section), reference fixture |
| 26 | A nested subalínea sequence is introduced with a colon and uses the required subordinate punctuation. | PASS | `abntexto-ufc.cls` (layout section), reference fixture |
...
| 30 | Unknown place/publisher data must not emit obsolete/inappropriate patterns for online resources; electronic examples must follow current NBR 6023 handling. | PASS — controlled reviewer case accepted. | `abntexto-ufc.cls` (NBR 6023:2025 compatibility section), bibliography fixtures |
