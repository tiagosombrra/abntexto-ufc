...
R1-BLOCK-7 restores permanent orchestration incrementally. B7-B established `Static contract`; B7-C1 certified the clean-runner `make check` contract; B7-C2 established bounded permanent PR orchestration through `Linux integration`; and B7-C3 is DONE with bounded `Linux release check` orchestration as a thin consumer of `make release-check`. PR #225 merged at `d7327db7efd5cc1e0ff9255195bcb9767d853d3e`, and permanent main run `33566835570` passed all 32 release checks (`PASS=32 FAIL=0 SKIP=0`). B7-D is active for residual workflow/status/ruleset audit and B8 handoff; B8 remains the owner of final Windows/literal-font/PDF-A certification.
