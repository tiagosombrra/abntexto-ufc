...
Updated: 2026-09-06  
Status: ACTIVE — STEP 4 FRONT-BLOCK ACTIVATION CORRECTION / ACCEPTANCE RERUN PENDING
...
| 4 | Textual structure and body typography | **RUNTIME CORRECTION IMPLEMENTED — CI PENDING** | latest physical failure `bf6c48e...`; implementation `3796a3c...` moves body activation to required front-block completion |
...
| `bf6c48e0cc1e5d19751ad2ff112db77fce799706` | Static `34071163701` PASS; Linux `34071163702` article PASS=4 FAIL=1; body still `20.700 pt` vs `13.800 pt` | **real runtime defect persists; `begindocument/end` route physically ineffective** |

The latest Linux run is decisive because validator-source, profile, required front block and optional foreign-element gates all passed. The body gate reached the unchanged physical checker and failed only the single-spacing predicate. No source-gate ambiguity remains.

## Current runtime correction — `3796a3c206adb7605160828dcda91f5851a16903`

The article body contract is now activated at the end of the required `\ufcPrintArticleFrontMatter` command, immediately after `\ufc_article_primary_summary:n {#1}`. This boundary occurs inside the document after shared startup initialization and before normal article body content.

The ineffective `begindocument/end` registration is removed. The dedicated body gate now verifies that body activation follows the primary summary and rejects reintroduction of the obsolete hook route.

Step 2 and Step 3 gates no longer inspect Step 4 implementation syntax. They continue to validate their own two-engine presentation/optionality contracts. This prevents cross-step gate coupling without weakening Step 4 because the body gate still enforces:

- profile-scoped activation;
- explicit `\singlesp` in the body function;
- 12 pt body size;
- 2 cm first-line indent;
- justified alignment;
- physical same-document single-spacing equivalence under pdfLaTeX and LuaLaTeX;
- required structure;
- missing-development negative rejection.

Article proof state is not promoted by this implementation change.
...
5. source evidence proves activation after the required primary summary and absence of the ineffective begin-document route;
...
| Active PR | #286 |
| Latest rejected physical checkpoint | `bf6c48e0cc1e5d19751ad2ff112db77fce799706` |
| Current runtime implementation | `3796a3c206adb7605160828dcda91f5851a16903` |

Next: publish the synchronized documentation checkpoint containing implementation `3796a3c...`, run Static plus bounded Linux `article`, classify any remaining failure fail-closed, and only after green acceptance advance to Step 5.
