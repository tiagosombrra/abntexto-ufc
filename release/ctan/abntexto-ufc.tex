...
\date{Version \version\ --- 2026-09-17}
...
The canonical package archive is \texttt{abntexto-ufc-3.0.3.zip}. It is the single archive intended for CTAN submission and contains one top-level \texttt{abntexto-ufc/} directory. Its only runtime implementation file is the generated monolithic \texttt{abntexto-ufc.cls}; the remaining files are README, CHANGELOG, license, this manual and the minimal example in source/PDF form.
...
Separate editable-template and Overleaf archives may be attached to the GitHub Release for user convenience. Those user bundles preserve the repository-oriented modular layout and include the project-provided UFC coat-of-arms asset used by the canonical example; they are not CTAN upload files.
