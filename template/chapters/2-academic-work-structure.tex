...
A organização de um trabalho acadêmico é normalmente compreendida em três grandes grupos: elementos pré-textuais, elementos textuais e elementos pós-textuais. O template separa essas etapas com os comandos \texttt{\string\pretextual} e \texttt{\string\textual}; referências, glossário, apêndices e anexos são produzidos depois do corpo principal. O índice remissivo permanece disponível como recurso opcional, mas não é impresso no documento canônico.
...
Pós-textuais & referências, glossário, apêndices e anexos; índice quando aplicável & complementam o trabalho depois do texto principal; a aplicabilidade varia conforme o elemento \\
\bottomrule
\end{tallabnttblr}
\end{table}
...
\guiamecanismo{A sequência concreta dos pré-textuais fica no próprio \texttt{main.tex}. Os comandos \texttt{\string\ufcPrintCover}, \texttt{\string\ufcPrintTitlePage}, \texttt{\string\ufcPrintApprovalPage} e os demais comandos \texttt{\string\ufcPrint...} mantêm a ordem visível explícita sem duplicar a diagramação de cada elemento.}
...
\guiamecanismo{Use os comandos estruturais do \LaTeX{}, como \texttt{\string\section} e \texttt{\string\subsection}, em vez de digitar manualmente números ou aplicar formatação visual aos títulos. A classe produz a apresentação e o sumário a partir dessa estrutura.}
...
\guianormativa{ABNT NBR 6034:2004}{O índice remissivo é um elemento pós-textual opcional. O documento canônico não o imprime por padrão; a classe mantém o recurso disponível para trabalhos em que ele seja útil \parencite{abnt6034}.}

\subsection{Paginação: visão estrutural}

Os pré-textuais fazem parte da progressão física do documento, enquanto a apresentação do número de página segue as regras aplicáveis à parte textual e ao modo de impressão. Em documentos frente e verso, a classe também precisa considerar o lado físico da folha e a abertura de elementos em página apropriada.

\guiamecanismo{A opção \texttt{print-mode} define a política de anverso único ou frente e verso. A posição, a exibição dos números e o espelhamento de margens são controlados pela classe; não devem ser simulados com quebras, cabeçalhos ou deslocamentos manuais.}

\guiavalidacao{Esta seção apresenta apenas a visão estrutural. As regras de geometria, número visível e comportamento frente e verso são consolidadas na Seção~\ref{sec:formatacao-geral} e protegidas pelos gates de layout, geometria de PDF e cenários duplex.}
