...
\guiamecanismo{Use \texttt{make compile} para o documento canônico. O alvo executa o motor LaTeX, chama Biber quando existe fonte bibliográfica ativa, chama \texttt{makeglossaries} quando há glossário e recompila o documento para estabilizar referências. Se o usuário habilitar o índice remissivo opcional com \texttt{index=imakeidx}, o mesmo fluxo chama \texttt{makeindex} quando necessário. \texttt{make check} executa o gate integrado de desenvolvimento; \texttt{make release-check} é reservado à matriz de release.}
...
\guianormativa{ABNT NBR 6034:2004}{O índice é elemento opcional e possui norma específica de apresentação. O PDF canônico não inclui um índice remissivo; a classe preserva a capacidade para o usuário habilitá-lo quando a natureza do trabalho justificar esse recurso \parencite{abnt6034}.}

\guiamecanismo{Quando necessário, adicione \texttt{index=imakeidx} ao \texttt{\string\ufcsetup}, marque os termos desejados com \texttt{\string\index} e chame \texttt{\string\ufcPrintIndex} ao final do documento. O fluxo de compilação executa \texttt{makeindex} quando o módulo estiver habilitado.}
