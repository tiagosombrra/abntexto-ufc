...
- B7-C1 certified the repository-owned `make check` contract on a clean TeX Live 2026 runner. B7-C2 is DONE and permanently provides `.github/workflows/linux-integration.yml` with stable `Linux integration` status. B7-C3 is DONE and permanently provides `.github/workflows/linux-release-check.yml`; PR #225 merged at `d7327db7efd5cc1e0ff9255195bcb9767d853d3e`, and permanent main run `33566835570` passed all 32 release checks (`PASS=32 FAIL=0 SKIP=0`). B7-D is ACTIVE for residual workflow/status/ruleset audit and B8 handoff. Final Windows/literal-font/PDF-A certification remains B8-owned.
