...
Final public/distribution ZIPs and `SHA256SUMS` must be generated by the permanent `Linux release check` from the same immutable Release candidate SHA that passes the phase-end regression. Pre-candidate builds may be diagnostic only; they are not publication assets. The release workflow must retain the generated `dist/` set as a downloadable GitHub Actions artifact so the later GitHub Release uses the exact certified bytes.

## Immediate Release discipline

1. keep the accepted transport checkpoint `6a257f35...` recorded;
2. add and validate durable `dist/` artifact upload to the permanent `Linux release check` without changing product/normative semantics;
...
5. verify the uploaded four ZIPs and `SHA256SUMS` from the accepted candidate;
6. only after acceptance, create/verify the `v3.0.0` tag, GitHub Release and any explicit documented publication;
