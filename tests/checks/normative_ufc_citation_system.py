...
def validate_fixture(name: str, fixture: Any) -> dict[str, Any]:
    required = {"citation_key", "source_tokens", "date_tokens"}
    ...
    if not isinstance(fixture.get("citation_key"), str) or not fixture["citation_key"]:
        fail(f"{name} citation_key must be a non-empty string")
    for key in ("source_tokens", "date_tokens"):
...
def measure_surface(
    ...
    source_presence = {
        token: normalize(token) in text for token in fixture["source_tokens"]
    }
    date_presence = {
        token: normalize(token) in text for token in fixture["date_tokens"]
    }
    author_date_present = all(source_presence.values()) and all(date_presence.values())
    return {
        "surface": name,
        "page": page.index,
        "fixture_citation_key": fixture["citation_key"],
        "source_tokens": source_presence,
        "date_tokens": date_presence,
        "author_date_present": author_date_present,
        "citation_window_word_count": len(words),
        "punctuation_order_and_surface_form_are_observational": True,
    }
...
def main() -> None:
    ...
    if locator_rule.get("rule_ids") != [RULE_ID]:
        fail("UFC citation-system locator rule scope drift")
    if locator_rule.get("status") != "VERIFIED":
        fail(f"UFC citation-system locator status drift: {locator_rule.get('status')}")
    ...
    passed_surfaces = sum(1 for surface in surfaces if surface["author_date_present"])
    rule_passed = passed_surfaces == len(surfaces)

    measured = {
        "system": "author-date" if rule_passed else "not-confirmed",
        "surface_count": len(surfaces),
        "author_date_surface_count": passed_surfaces,
        "surfaces": surfaces,
        "positive_fixture_evidence_only": True,
        "exact_citation_format_not_strengthened": True,
    }
    evidence = [
        {
            "rule_id": RULE_ID,
