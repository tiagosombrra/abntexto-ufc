...
if [ -n "$warnings" ]; then
  printf '%s\n' "$warnings"
  echo "Auditoria N7 de paginação falhou: warning ou overflow não reconhecido em $fixture."
  exit 1
fi
...
test -s "$evidence" || {
  echo 'Auditoria N7 de paginação falhou: evidência JSON não foi gerada.'
  exit 1
}

echo 'Gate de evidência N7 para geometria da paginação concluído.'
