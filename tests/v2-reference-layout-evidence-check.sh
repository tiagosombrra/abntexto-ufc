...
if [ -n "$warnings" ]; then
  printf '%s\n' "$warnings"
  echo "Auditoria N8 de layout de referências falhou: warning ou overflow não reconhecido em $fixture."
  exit 1
fi

mkdir -p "$(dirname "$evidence")"
python3 tests/checks/normative_reference_layout.py \
  "$job.pdf" \
  --json "$evidence" \
  --commit-sha "${SOURCE_COMMIT_SHA:-${GITHUB_SHA:-}}"

test -s "$evidence" || {
  echo 'Auditoria N8 de layout de referências falhou: evidência JSON não foi gerada.'
  exit 1
}

echo 'Gate de evidência N8 para layout de referências concluído.'
