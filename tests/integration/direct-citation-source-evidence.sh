...
if [ -n "$warnings" ]; then
  printf '%s\n' "$warnings"
  echo "Auditoria de atribuição de fonte em citação direta falhou: warning ou overflow não reconhecido em $fixture."
  exit 1
fi
...
test -s "$evidence" || {
  echo 'Auditoria de atribuição de fonte em citação direta falhou: evidência JSON não foi gerada.'
  exit 1
}

echo 'Gate de evidência N6 para atribuição de fonte em citação direta concluído.'
