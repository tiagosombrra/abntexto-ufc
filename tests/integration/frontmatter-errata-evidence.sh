...
compile_fixture() {
  ...
  if [ -n "$warnings" ]; then
    printf '%s\n' "$warnings"
    echo "Errata audit failed: unrecognized warning or overflow in $fixture."
    exit 1
  fi
}
...
test -s "$evidence" || {
  echo 'Errata audit failed: JSON evidence was not generated.'
  exit 1
}

echo 'Evidence front matter for errata gate completed.'
