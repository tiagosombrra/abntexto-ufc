...
if [ -n "$warnings" ]; then
  printf '%s\n' "$warnings"
  echo "Audit for source em indirect citation failed: unrecognized warning or overflow em $fixture."
  exit 1
fi
...
test -s "$evidence" || {
  echo 'Audit for source em indirect citation failed: JSON evidence was not generated.'
  exit 1
}

echo 'Gate for evidence for source em indirect citation completed.'
