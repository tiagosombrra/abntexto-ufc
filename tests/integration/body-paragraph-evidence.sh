...
if [ -n "$warnings" ]; then
  printf '%s\n' "$warnings"
  echo "Audit for parágrafos do corpo failed: unrecognized warning or overflow em $fixture."
  exit 1
fi
...
test -s "$evidence" || {
  echo 'Audit for parágrafos do corpo failed: JSON evidence was not generated.'
  exit 1
}

echo 'Gate for evidence for parágrafos do corpo completed.'
