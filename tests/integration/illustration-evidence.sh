...
if [ -n "$warnings" ]; then
  printf '%s\n' "$warnings"
  echo "Auditoria de ilustrações falhou: warning ou overflow não reconhecido em $fixture."
  exit 1
fi
...
test -s "$evidence" || {
  echo 'Auditoria de ilustrações falhou: evidência JSON não foi gerada.'
  exit 1
}

echo 'Gate de evidência para ilustrações concluído.'
