...
if [ -n "$warnings" ]; then
  printf '%s\n' "$warnings"
  echo "Auditoria de recuo suspenso de títulos de seção falhou: warning ou overflow não reconhecido em $fixture."
  exit 1
fi
...
test -s "$evidence" || {
  echo 'Auditoria de recuo suspenso de títulos de seção falhou: evidência JSON não foi gerada.'
  exit 1
}
